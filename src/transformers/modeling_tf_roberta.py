...
@add_start_docstrings(
    """RobBERTa Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear layers on top of
...
class TFRobertaForQuestionAnswering(TFRobertaPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs):
        super(TFRobertaForQuestionAnswering, self).__init__(config, *inputs, **kwargs)
        self.num_labels = config.num_labels
